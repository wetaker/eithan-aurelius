...
class SubscriberCollection {
    ...
    addSubscriber(sub : Subscriber) : void {
        this.subscribers.set(sub.tag, sub)
    }

    removeSubscriber(sub : Subscriber) : void {
        this.subscribers.delete(sub.tag)
    }

    subscribed?(sub : Subscriber) {
        return this.subscribers.has(sub.tag)
    }
    ...
    update(user_tag : Tag, channel_name : Channel, duration : number) {
        ...
        data.channels.set(channel_name, (data.channels.get(channel_name) | 0) + duration)

        if (data.work_by_day.has(today))
            data.work_by_day.get(today).duration += duration
        else
            data.work_by_day.set(today, {channel:channel_name, duration:duration})
    }

}

export {SubscriberCollection}
